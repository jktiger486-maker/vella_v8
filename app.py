...
CFG = {
    ...
    # =====================================================
    "12_EXECUTION_MIN_PRICE_MOVE_PCT": 0.05,
    ...
    # - 손익곡선 흔들림 감소
    # 판단:
    # - 현재 CFG 12 + 13 + 21 + 37 조합과 궁합 최상
    # - “운영용 메인 버전”으로 가장 안전한 선택

    "37_TRAILING_PCT": 0.30,
    # 추천 테스트값: 
    # 0.30(추세는 충분히 먹고 되돌림 초입에서 이익 고정)
    # 0.40(추세 끝까지 더 끌고 감, 되돌림에 이익 반납 자주 발생)
    # 0.20(스캘핑 성향, TP 찍자마자 조금만 튀면 바로 종료, 승률 ↑ 평균 수익 ↓)
    # 의미:
    # - 트레일링 스탑 민감도
    # 효과:
    # - 익절 후 되돌림에서 이익 더 빨리 고정
    # - 손익 곡선 변동성 감소
    # 특징:
    # - ENTRY 횟수 영향 ❌
    # - EXIT 타이밍만 미세 조정
